...
def get_session_table():
    ...
    df.dropna(subset=['session'], inplace=True) # Remove rows with no session number (only leave the nwb file with the largest finished_trials for now)
    df.drop(df.query('session < 1').index, inplace=True)

    # Remove invalid subject_id
    df = df[(999999 > df["subject_id"].astype(int)) 
    ...
    if 'bpod_backup_h2o' in df.columns:
        df['subject_alias'] = np.where(df['bpod_backup_h2o'].notnull(), df['bpod_backup_h2o'], df['subject_id'])
        df['trainer'] = np.where(df['bpod_backup_user_name'].notnull(), df['bpod_backup_user_name'], df['trainer'])
    else:
    ...
    # trial stats
    df['avg_trial_length_in_seconds'] = df['session_run_time_in_min'] / df['total_trials_with_autowater'] * 60
